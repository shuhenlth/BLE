...
Page({
  data: {
    ...
    service2_command1: '',//按钮“指令1”中的存储用户设置的字符
    service2_command2: '',//按钮“指令2”中的存储用户设置的字符
  ...
  onLoad(options) {
    ...
    var value = wx.getStorageSync('device2')//从本地缓存中同步获取指定 key 的内容
    if (value) {
    ...
    var command = wx.getStorageSync('service2_command1')//从本地缓存中同步获取指定 key 的内容
    if (command) {
      this.setData({
        service2_command1: command,
      })
    }

    var command = wx.getStorageSync('service2_command2')//从本地缓存中同步获取指定 key 的内容
    if (command) {
      this.setData({
        service2_command2: command,
      })
    }
  },
            ...
            success(res) {
              if (res.confirm) {//用户点击确定
                if (res.content.length > 0) {//用户有输入内容
                  wx.setStorageSync('service2_command1', res.content)
                  that.setData({
                    service2_command1: res.content
                  })
                  wx.showToast({
                    title: '设置成功',
                  })
        ...
        if (res.confirm) {//用户点击确定
          wx.showModal({
            ...
            success(res) {
              if (res.confirm) {//用户点击确定
                if (res.content.length > 0) {//用户有输入内容
                  wx.setStorageSync('service2_command2', res.content)
                  that.setData({
                    service2_command2: res.content
                  })
                  wx.showToast({
                    title: '设置成功',
                  })
  ...
  //点击,发送之前设置的内容
  goCommand1() {
    let value = this.data.service2_command1
    if (value === '') {//指令为空
      wx.showToast({
        icon: 'none',
        title: '未设置内容，发送失败！'
  ...
  goCommand2() {
    let value = this.data.service2_command2
    if (value === '') {//指令为空
      wx.showToast({
        icon: 'none',
        title: '未设置内容，发送失败！'
  ...
  cycleSend() {
    ...
    if (this.data.isCycleSend) {//开启了循环发送
      ...
      app.globalData.device2Timer = setInterval(function () {//开启定时器
        that.send()
      }, parseInt(this.data.cycle))//字符串转数字
    } else {
      clearInterval(app.globalData.device2Timer)//关闭定时器
